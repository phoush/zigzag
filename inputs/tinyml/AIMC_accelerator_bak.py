...
def multiplier_array():
    ...
    multiplier_input_precision = [4, 4] # this value is not used in model
    AIMC_unit_costs = {'vdd' : 0.8, # power supply voltage
            'ADC_ENOB' : 4, # output precision after ADC, corresponding to partial sum precision
            'DAC_RES' : 1,  # DAC input precision (#bits/cycle per DAC). (#cycle number) inferred from here.
            'WEIGHT_BITCELL' : 4, # equal to weight precision for now
            'BL_PER_ADC': 1} # (#bitlines/ADC), set to 1 for now

    AIMC_area = {'cell': 1, 'ADC': 1, 'DAC': 1}
    dimensions = {'D1': 1024, 'D2': 64}
    technology = 22
