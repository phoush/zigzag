...
def multiplier_array():
    ...
    AIMC_unit_costs = {'vdd' : 0.8, # power supply voltage
            'ADC_RES' : 8, # output precision after ADC, corresponding to partial sum precision
            'DAC_RES' : 4,  # DAC input precision (#bits/cycle per DAC). (#cycle number) inferred from here.
            'WEIGHT_BITCELL' : 8, # equal to weight precision for now
            'BL_PER_ADC': 1} # (#bitlines/ADC), set to 1 for now

    AIMC_area = {'cell': 1, 'ADC': 1, 'DAC': 1}
    dimensions = {'D1': 256, 'D2':1152, 'D3':1}
    technology = 28
