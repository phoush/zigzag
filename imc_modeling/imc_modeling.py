...
def aimc_model():
    ...
    def get_cap_cell(x):
        ...
        return cap_cell[x]
    def get_adc_conv(x):
        adc_conv_step = {'28':0.1e-12, '16':0.1e-12, '65':0.1e-12, '12':0.1e-12, '22':0.1e-12, '7':0.1e-12, '55':0.1e-12, '180':0.1e-12, '45':0.1e-12, '0': 0.1e-12}
...
#    t['Emac_murmann'] = (1/(t['Rows'] )) * t['Eadc_murmann']
    t['Ecap_murmann'] = activity_cell * t['Unit capacitance'] * np.power(t['Weight precision'], 1)
    t['Elogic_murmann'] = 0.1 * 0.3e-15 * np.power(t['Weight precision'], 1) * 4
    t['Emac_murmann'] = (1/(t['Rows'] )) * t['Eadc_murmann'] / np.power(t['Voltage (V)'],2)
 
    # Compute TOPSW/1B
    t['TOPSW_zimmer'] = 2/ t['Emac_zimmer'] / 1e12
    t['TOPSW_murmann'] = 2/ (t['Elogic_murmann'] + t['Emac_murmann'] + t['Ecap_murmann']) / t['Div factor'] /  1e12
    t['Cap%_murmann'] = t['Ecap_murmann'] / (t['Ecap_murmann'] + t['Emac_murmann'])
    ...
    scatter_labels, topsw_paper, topsw_zimmer, topsw_murmann, ecap_ratio  = [], [], [], [], []
    for i,r in t.iterrows():
        ...
        ecap_ratio.append(r['Cap%_murmann'])
...
#        customdata=np.stack((topsw_paper, topsw_zimmer), axis=-1), hovertemplate='x:%{x}<br>y:%{y}<br>%{text}<br>TOPs/W paper %{customdata[0]:.2f}<br>TOPs/W Zimmer %{customdata[1]:.2f}'), row=1,col=2)
    fig.add_trace(go.Scatter(x=t['Idx'], y=t['Murmann_diff'],mode='markers+text',text=scatter_labels, name='Murmann / Paper', 
        customdata=np.stack((topsw_paper, topsw_murmann, ecap_ratio), axis=-1), hovertemplate='x:%{x}<br>y:%{y}<br>%{text}<br>TOPs/W paper %{customdata[0]:.2f}<br>TOPs/W Murmann %{customdata[1]:.2f}<br>Cap ratio %{customdata[2]:.2f}'), row=1, col=2)
